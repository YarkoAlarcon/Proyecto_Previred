...
public class Enfermo_terminal extends CC_Test {
			...
			String afeccionoenfermedad, String xNumero) 
	{
		...
		try {
			...
			// Verifica si se llegó a la pantalla "Antecedentes laborales"
			WebElement labelAntecedentesElement = elementFetch.getWebElement("XPATH",
			        "//h3[text()='Antecedentes laborales']");
			wait.until(ExpectedConditions.visibilityOf(labelAntecedentesElement));
			String textoPantalla = labelAntecedentesElement.getText();

			GG_Validations.trueBooleanCondition(textoPantalla.equalsIgnoreCase("Antecedentes laborales"),
			        "Se ha ingresado correctamente a la pantalla: " + textoPantalla,
			        "No se ha ingresado correctamente a la pantalla: ", currentEvent);
